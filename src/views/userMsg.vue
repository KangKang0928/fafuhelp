<template>
  <div class="useMsg">
    <div class="main">
      <ul class="msg">
        <form action="userSubmitOrder.php" method="POST" class="userSubmit">
          需求类型:
          <select name="keyWord">
          ...
          <textarea
            ...
            rows="10"
            required="required"
            placeholder="填写您的隐私信息，例如联系方式，快递单号，宿舍号等QQ:888888 快递单号:SF8888"
          ></textarea>
          需求描述/公开信息:
          <textarea
            name="requireDec"
            ...
            rows="10"
            required="required"
            placeholder="填写您的公开信息，例如:需要一个人领快递~有偿"
          ></textarea>
          <input type="submit" value="立即发送" class="float_right" />
        </form>
      </ul>
    </div>
  </div>
...
.order .msg>li,.order .msg>li>form{
    width: 100%;
    height:auto;
    display: flex;
    justify-content:space-around;
...
.main .msg input,.order .msg input{
    border: none;
    outline: none;
    padding: 10px;
...
@media screen and (max-width:991px){
    ...
    .final_msg:nth-child(2) p,.final_msg:nth-child(1) p{
        text-align: center;
        font-size: 12px;
    }
	...
	.main .msg>li>form,.order .msg>li>form{
    padding: 0;
	}
	.order .msg>li,.order .msg>li>form{
		box-shadow: 0px 0px 0px rgba(0,0,0,0);
